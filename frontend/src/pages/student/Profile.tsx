...
function InfoRow({ icon, label, value }: InfoRowProps) {
  return (
    <div className="flex items-center gap-4 py-3.5 border-b border-slate-200 dark:border-slate-800 last:border-0">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-sm text-slate-900 dark:text-white font-bold mt-0.5 truncate">{value ?? '—'}</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-slate-600 dark:text-slate-300 text-sm font-medium mt-1">Your KLU account and LMS synchronization details</p>
      </div>

      {/* Avatar card */}
      <Card className="flex flex-col sm:flex-row items-center sm:items-start gap-5">
        <div className="w-20 h-20 rounded-3xl bg-gradient-to-br from-violet-600 to-indigo-600 flex items-center justify-center text-3xl font-black text-white shadow-xl shadow-indigo-950/30 dark:shadow-indigo-950/60 shrink-0">
          {initialLetter}
        </div>
        <div className="flex-1 text-center sm:text-left space-y-1">
          <h3 className="text-xl font-black text-slate-900 dark:text-white leading-tight">{displayName}</h3>
          {student?.studentId && (
            <p className="text-indigo-600 dark:text-indigo-300 text-xs font-mono font-bold">{student.studentId}</p>
          )}
          <div className="flex items-center justify-center sm:justify-start gap-2 pt-2">
            <Badge status={student?.status || 'active'} size="md" />
            <span className="text-xs text-indigo-700 dark:text-indigo-300 bg-indigo-50 dark:bg-indigo-950/80 border border-indigo-200 dark:border-indigo-500/30 px-2.5 py-0.5 rounded-full font-mono font-bold">
      ...
      {/* Info card */}
      <Card>
        <h3 className="text-xs font-bold text-slate-700 dark:text-slate-200 uppercase tracking-wider mb-2">Account Details</h3>

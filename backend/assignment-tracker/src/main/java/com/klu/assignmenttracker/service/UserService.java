...
public class UserService {
     ...
     * @return JWT token and student profile
     */
    public AuthResponse studentLogin(StudentLoginRequest request) {
        // ── Step 1: Authenticate with KLU Moodle Web Services ────────────────
        String moodleToken = lmsAuthService.authenticateAndGetToken(
                request.getStudentId(), request.getLmsPassword());

        // ── Step 2: Find or create the student record ────────────────────────
        User user = userRepository.findByStudentId(request.getStudentId())
                .orElseGet(() -> {
                    log.info("First login for studentId={}; creating new student record.",
        ...
        // ── Step 3: Store Moodle token in memory cache ───────────────────────
        moodleTokenCache.storeToken(user.getId(), moodleToken);
        moodleTokenCache.storeToken(user.getStudentId(), moodleToken);

        // ── Step 4: Synchronize Courses and Assignments from Moodle ──────────
        try {
            syncService.syncUserAssignments(user, moodleToken);
        } catch (Exception e) {
            log.warn("Moodle assignment sync during login failed for studentId={}: {}",
                    user.getStudentId(), e.getMessage());
        }

        // ── Step 5: Update lastLogin & save user ─────────────────────────────
        user.setLastLogin(LocalDateTime.now());
        User savedUser = userRepository.save(user);
        log.info("Student login successful: studentId={}", savedUser.getStudentId());
